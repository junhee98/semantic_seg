...
# if you want to train & inference SegNet_CReLU, use it.
'''
class SegNet_CReLU(BaseModel):
    def __init__(self, num_classes, in_channels=3, pretrained=True, freeze_bn=False, freeze_backbone=False, **_):
        super(SegNet_CReLU, self).__init__()
...
'''
# if you want to train & inference SegNet_Light_CReLU, use it.
...
class SegNet_CReLU(BaseModel):
    ...
    def forward(self, x):
        ...
        res = self.stage1_1_encoder_1conv(res)
        x1_size = res.size()
        out_1 = self.act(res).clone()
        x, indices1 = self.pool(out_1)

        res = self.stage2_1_encoder(x)
        res = self.stage2_1_encoder_1conv(res)
        x2_size = res.size()
        out_2 = self.act(res).clone()
        x, indices2 = self.pool(out_2)

        res = self.stage3_1_encoder(x)
        res = self.stage3_1_encoder_1conv(res)
        x3_size = res.size()
        out_3 = self.act(res).clone()
        x, indices3 = self.pool(out_3)
